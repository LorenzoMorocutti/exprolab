...
#   complete and consistent
    try:
        ...
        # the ID of the hyp in the array temp_int and then publish the IDs (if there are any) 
        for i in completed:
            find = False
...
#   (in my case: /root/ros_ws/src).
    ...
    try:
        ...
        req.args= ['/root/ros_ws/src/exprolab_ass2/cluedo_ontology.owl', 'http://www.emarolab.it/cluedo-ontology', 'true', 'PELLET', 'true']
...
#    with the disjuncted property (we have to explicitly say two entities are separate). 
    ...
    try:
        # from the class_type (who,what,where) find the entity_class (PERSON,WEAPON,LOCATION)
