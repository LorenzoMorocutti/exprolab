...
std::vector<exprolab_ass2::ErlOracle> oracle_msgs;
...
bool oracleService(exprolab_ass2::Oracle::Request &req, exprolab_ass2::Oracle::Response &res)
	{
		res.ID = winID;
		return true;
	}
   ...
   for(int i=0; i< msg->name.size(); i++){
	   if (msg->name[i].find("cluedo_link")!= std::string::npos){
		   for(int j=0; j<4;j++){
				...
				exprolab_ass2::ErlOracle oracle_msg;
...
oracle_pub = nh.advertise<exprolab_ass2::ErlOracle>( "/oracle_hint", 0 );
